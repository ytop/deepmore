...
- [x] 1. Extend logging types and add `fast-check` for property-based testing
  - [x] 1.1 Add `fast-check` as a devDependency
    - Edit `deepseek-telegram-agent/package.json` to add `fast-check` (recent stable version) under `devDependencies`
    - Run `bun install` in `deepseek-telegram-agent/` to populate `bun.lock`
...
- [x] 2. Implement the iteration cap in `Agent.runLoop`
  - [x] 2.1 Add `resolveMaxIterations` and the module-level `RESOLVED_MAX_ITERATIONS` constant
    - Edit `deepseek-telegram-agent/agent.ts`: at module top (above `class Agent`), add the constants `DEFAULT_MAX_ITERATIONS = 25`, `MAX_ITERATIONS_LOWER_BOUND = 1`, `MAX_ITERATIONS_UPPER_BOUND = 1000`
  ...
  - [ ]* 2.2 Property test for `resolveMaxIterations` purity
    ...
    - Create `deepseek-telegram-agent/agent.test.ts` with the workflow tag `// Feature: cap-agent-tool-loop, Property 1: …`
  ...
  - [ ]* 2.3 Property test for the resolver's stderr warning on invalid input
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 2: …`
  ...
  - [x] 2.4 Add the `maxIterations` field on `Agent` and the cap branch in `runLoop`
    - Edit `deepseek-telegram-agent/agent.ts`: declare `private readonly maxIterations: number;` on the `Agent` class and assign `this.maxIterations = RESOLVED_MAX_ITERATIONS` in the constructor (so any post-construction mutation of `process.env` does not affect the value)
  ...
  - [ ]* 2.5 Property test for `Cap_Reply` exact formatting
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 5: …`
...
- [x] 3. Build the test-only `agent.legacy.ts` reference fixture for Property 4
  - [x] 3.1 Extract the pre-feature `runLoop` into `agent.legacy.ts`
    - Create `deepseek-telegram-agent/agent.legacy.ts` exporting a function (or class) that runs the **pre-feature** loop body — identical to the current `runLoop` shape but with **no** cap branch, no `loop_cap` emission, and no `maxIterations` field
...
- [ ] 4. Property tests for cap behaviour in `agent.test.ts`
  - [ ]* 4.1 Property test that the cap value is fixed and the iteration counter is per-call
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 3: …`
  ...
  - [ ]* 4.2 Property test that below-cap behaviour matches the pre-feature implementation
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 4: …`
  ...
  - [ ]* 4.3 Property test that the cap halts the loop at iteration `N` and `sendMessage` resolves to `Cap_Reply`
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 6: …`
  ...
  - [ ]* 4.4 Property test that the capping assistant message is the last entry in history
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 7: …`
  ...
  - [ ]* 4.5 Property test that the `loop_cap` history entry records the cap event faithfully
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 8: …`
  ...
  - [ ]* 4.6 Property test that a logging failure on the `loop_cap` append does not propagate
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 9: …`
  ...
  - [ ]* 4.7 Property test that event ordering on every post-first iteration is `retry` first, with cap tail `..., retry, loop_cap`
    ...
    - Add to `deepseek-telegram-agent/agent.test.ts` with tag `// Feature: cap-agent-tool-loop, Property 10: …`
...
- [x] 5. Checkpoint — agent + logger work end-to-end
  - Ensure `bun test deepseek-telegram-agent/agent.test.ts log/logger.test.ts` runs cleanly with all property tests passing (those marked `*` are optional but should be green when implemented). Ensure all tests pass, ask the user if questions arise.
...
- [x] 6. Document the new env var and add example-based Telegram wiring tests
  - [x] 6.1 Document `AGENT_MAX_ITERATIONS` in `.env.example`
    - Append to `deepseek-telegram-agent/.env.example` exactly one commented line documenting the optional variable, matching the file's existing style for optional vars: `# AGENT_MAX_ITERATIONS=25  # Optional. Max tool-loop iterations per turn (1-1000). Default 25.`
  ...
  - [ ]* 6.2 Example-based test: `Cap_Reply` is delivered to Telegram via `MessageBatcher.enqueue`
    ...
    - Create `deepseek-telegram-agent/index.test.ts` (example-based, not property-based — this is wiring, not behaviour-over-input)
  ...
  - [ ]* 6.3 Example-based test: enqueue failure for `Cap_Reply` falls into the existing `catch (error: any)` path without crashing
    ...
    - Add to `deepseek-telegram-agent/index.test.ts`
...
- [x] 7. Final checkpoint — full test suite green
  - Ensure `bun test` from the repo root (or `bun test` inside `deepseek-telegram-agent/`) passes for every non-optional task and every optional `*` task that was implemented. Ensure all tests pass, ask the user if questions arise.
